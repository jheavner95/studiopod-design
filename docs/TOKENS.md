...
```bash
npm install @jheavner95/design@^0.1.0
```

```css
/* app/globals.css — order matters; later @theme wins */
@import 'tailwindcss';
@import '@jheavner95/design/styles.css';   /* palette, semantic tokens, typography, utilities */
@source '../node_modules/@jheavner95/design/dist';  /* REQUIRED — see below */
```

```ts
import { motionDuration, motionEase, zIndex, accentRgb, successRgb } from '@jheavner95/design/tokens';
```

**The `@source` line is not optional.** Tailwind ignores `node_modules` by default; without it, none of the package's components' Tailwind classes are generated, and every consumer of `@jheavner95/design` renders unstyled with no error. This is the single most common integration mistake — see `DISTRIBUTION.md` §3.3 and §5 for the full installation/upgrade guide, including what to re-verify on every version bump (tokens can restyle thousands of call sites with no app-side diff — always diff `dist/styles.css` between versions before upgrading).

No internal path is ever required. A consumer never imports `@jheavner95/design/src/styles/theme.css` or reaches into `dist/` directly — `/tokens` and `/styles.css` are the only two token-bearing entry points, both declared in `package.json`'s `exports` map.
