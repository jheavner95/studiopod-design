import { ProductionPipelineDiagram } from "@/production";
 ...
 * spacing, mobile stacks the stages automatically.
 */
export function ResponsiveSection() {
  return (
    <PreviewSection
      id="production-responsive"
      eyebrow="responsive behavior"
      title="Desktop, tablet, mobile"
      description="The same export validation chain at three container widths, readability maintained at every size."
    >
      <div className="flex flex-col gap-10">
        {FRAMES.map((frame) => (
          <div key={frame.label} className="flex flex-col gap-3">
            ...
            >
              <ProductionPipelineDiagram pipeline={exportValidation} nodeSize="sm" />
            </div>
          </div>
        ))}
      </div>
