...
export const TABLE_FUTURE_EXTENSIONS: TableFutureExtension[] = [
  ...
  { title: "Column Resize", description: "Drag-to-resize column widths, with the result persisted per user — not included because it needs pointer-tracking state this system doesn't own." },
  ...
  { title: "Inline Editing", description: "Editing a cell's value directly in the table, without opening the record in an Inspector — the highest-complexity extension on this list." },
  {
    ...
    description:
      "Switching a wide table to a stacked, card-per-row layout below a breakpoint, instead of relying on horizontal scroll. Table's ScrollArea handles overflow correctly but has no mechanism to switch to a stacked layout below a breakpoint — the gap MaturityTable and InventoryTable are both blocked on today.",
  },
];
