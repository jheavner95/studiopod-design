...
export const MACHINE_STATES: MachineStateNote[] = [
  ...
  { state: "Terminal", note: "StateNode's \"terminal\" value shows a filled square marker — the machine has reached an end state with no further transitions out of it. This value has no counterpart in Workflow Framework's own status model, since a plain workflow doesn't need to distinguish a true dead end from Completed the way a state machine does." },
];
