...
import { ASSET_WORKSPACE_REGIONS, type AssetWorkspaceRegion } from "../_data/regions";

interface RegionCardProps {
  region: AssetWorkspaceRegion;
  selected: boolean;
  onSelect: () => void;
}
  ...
  return (
    <button type="button" onClick={onSelect} aria-pressed={selected} className="focus-ring block w-full rounded-lg text-left">
      <Activate state={selected ? "active" : "inactive"} className="rounded-lg">
        <Card
          interactive
          className={cn("flex h-full min-h-24 flex-col gap-2", selected && "border-accent-500/60 bg-accent-soft/30")}
        >
...
function RegionDetail({ region }: { region: AssetWorkspaceRegion }) {
  return (
...
/** The Asset Workspace's four regions, stacked top to bottom exactly as the anatomy diagram shows — select one to see its full detail below. */
export function AssetWorkspaceAnatomyExplorer() {
  const [selectedId, setSelectedId] = useState(ASSET_WORKSPACE_REGIONS[0].id);
  const selected = ASSET_WORKSPACE_REGIONS.find((region) => region.id === selectedId) ?? ASSET_WORKSPACE_REGIONS[0];
