...
export const BREAKPOINT_NOTES: BreakpointNote[] = [
  ...
  { breakpoint: "Mobile", note: "The same horizontal scroll holds rather than collapsing into cards — DataGrid inherits Table's confirmed, twice-validated gap here (see Implementation Guidance): there's no responsive row-collapse capability yet, so a genuinely mobile-first grid should keep its column count small rather than expect one to appear." },
];
...
export const RESPONSIVE_TOPICS: ResponsiveTopic[] = [
  {
    ...
    note: "DataGrid never wraps or hides columns on its own — it inherits Table's independent horizontal ScrollArea, the same proven behavior every certified Table consumer already relies on.",
  },
  {
