...
export const COMMERCE_FUTURE_EXTENSIONS: CommerceFutureExtension[] = [
  ...
  { title: "Marketplace synchronization", description: "Real push/pull sync between the catalog and external marketplaces — this audit confirmed no real provider-sync integration exists anywhere in the repo (see Promotion Candidates), so this is genuinely greenfield, deferred pending real usage." },
  ...
  { title: "Order forecasting", description: "Predicting order volume from historical data — belongs to the Intelligence platform template (DS-4.1's own architecture), not this one; deferred here rather than duplicated." },
