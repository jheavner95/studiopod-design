...
export const PROMOTION_METHODOLOGY_NOTE =
  "Every one of the nine Operational systems dispatched its own dedicated duplication audit before writing new code, and every one of those audits is still published at that system's own _data/promotion-candidates.ts file. This section reclassifies their combined output — it does not re-run the searches, since the nine per-system audits behind this certification already re-verified each system's own reuse claims independently.";
