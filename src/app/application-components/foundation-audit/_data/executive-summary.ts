...
export const MIGRATION_PRIORITIES: string[] = [
  "1. ResponsiveRulesTable → Table (Low effort, 3 files) — COMPLETED",
  "2–3. Description Lists and their Accessibility blocks → DescriptionList (Low effort, 9 files) — COMPLETED",
...
export const CERTIFICATION_RECOMMENDATION =
  "Foundation Table: Certified. Table is the first family in this audit to clear all three of its own documented Certified criteria — Production Ready baseline, a verified per-component accessibility pass, and real, non-playground screen usage (5 pages, via ResponsiveRulesTable, ScorecardTable, CertificationMatrix, and CoverageMatrix). Getting there required one real API extension (TableHead's sticky prop, added during the ResponsiveRulesTable migration) that then held stable across three further migrations with zero further changes, plus two correctly-reverted attempts (MaturityTable, InventoryTable) that found and documented a real, twice-confirmed capability gap rather than shipping a regression — now recorded as a Future Extension, not a blocker to Table's own certification. Two minor, non-blocking accessibility gaps were found and disclosed (TableLoadingState and TableToolbar both lack a screen-reader state-change announcement). The other three built families, plus the catalog that tracks them, remain Production Ready, Adoption In Progress or not yet adopted. Recommendation: run the same pilot pattern for Forms (ControlDock → SwitchField is the lowest-risk candidate) and give Metadata its own accessibility pass and further adoption before pursuing Certified status for either — then begin Overlays, Navigation, and Feedback with the same discipline this layer was built with.";
