...
export const COMPOSITION_RULES: ComplianceCheck[] = [
  ...
  },
  {
    ...
    evidence:
      "Structurally nothing prevents it — all three sibling families are self-contained and depend only on Layout + ui. But there is zero real evidence of it happening: every one of the 9 Workspace Architecture pages imports only PageShell/SectionShell/CardGrid from Layout, and none imports anything from Table, Metadata, or Forms. No operational or workspace page has ever consumed these three families.",
  },
];
...
export const DEPENDENCY_LAYERS: DependencyLayer[] = [
  ...
  },
  {
    ...
    note: "Every page imports exactly the same 3 names from Layout and nothing else. Zero pages import Panel, Surface, or DescriptionList from that same module, and zero import anything from Table, Metadata, or Forms — despite those exact patterns being hand-rolled inline on 8 of the 9 pages.",
  },
];

export const COMPOSITION_SUMMARY =
  "The Foundation Layer is internally well-composed: Table, Metadata, and Forms each build cleanly on Layout and ui, never on each other, and both hard behavioral rules (Metadata never edits, Forms never present) hold with zero exceptions under direct grep. What the composition review does not find is any evidence the next layer up — Workspace Architecture — has started consuming any of it. Adoption stops at the page-shell level.";
