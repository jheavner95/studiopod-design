...
export const CERTIFICATION_HEADLINE =
  "Foundation Table reached Certified in DS-2.1.8 — the first family in this audit to clear all three of its own documented Certified criteria: Production Ready baseline, a verified per-component accessibility pass, and real, non-playground screen usage (5 pages, via ResponsiveRulesTable, ScorecardTable, CertificationMatrix, and CoverageMatrix). Getting there took four shipped migrations and two honestly reverted ones (MaturityTable, InventoryTable — a real capability gap, documented rather than forced through). The other three built families, plus the catalog that tracks them, remain at Production Ready. Metadata shows real adoption in progress (DescriptionList, DS-2.1.7.1) but hasn't yet run its own accessibility pass or adopted the other 15 components; Forms and Layout have real, disclosed gaps of their own. This is stated plainly rather than rounded up: one family has now fully proven itself under real usage — the rest are real and well-built, but not yet there.";
