...
export const QUEUE_CLEAN_FINDINGS: string[] = [
  ...
  "grep for JobCard|JobProgress|JobRetry|QueueRow|QueueStatus|retryJob|jobStatus and priority.*Badge|PriorityBadge|jobPriority across src/**/*.tsx: no independent implementation exists — QueuePriority in particular is genuinely new ground, no priority-badge pattern existed anywhere before this package.",
  "DS-0.2 inventory (inventory.ts, section \"4. Workflow Operations\") already tracks several of this package's gaps honestly: \"Queue Table\" (Needed, explicitly noting QueueFlow.tsx is \"a decorative animation of items moving along a track, not a data table\" — filled by Queue/QueueRow), \"Job Status Card\" (Partial, source WorkflowCard.tsx, \"the summarize-one-item-as-a-card pattern is well established, just not for a literal job\" — filled by JobCard), \"Batch Action Bar\" (Needed — filled by this package composing Bulk Actions directly), \"Progress Indicator\" (Exists, source motion/primitives/Progress.tsx, \"reusable but not job-specific\" — filled by JobProgress), \"Activity Timeline\" (Partial — filled by JobTimeline), \"Approval Panel\" (Needed, not addressed by this package — a genuinely separate future concern).",
];
