...
export const RESOLVED: PromotionEntry[] = [
  ...
  },
  {
    ...
    detail: "Rather than rebuild a direction-arrow edge and a node-cluster boundary a second time, WorkflowEdge and WorkflowGroup are literal re-exports of DS-3.7's own DependencyEdge/DependencyGroup — confirmed by direct source read, not restated from docs.",
  },
  {
...
export const REJECTED: PromotionEntry[] = [
  ...
  },
  {
    ...
    system: "Dependency & Relationship Views (DS-3.7)",
...
export const PROMOTION_METHODOLOGY_NOTE =
  "Every one of DS-3.1 through DS-3.8 dispatched its own dedicated duplication audit before writing new code, and every one of those audits is still published at its own package's _data/promotion-candidates.ts. This section reclassifies their combined output — it does not re-run the searches from scratch, since the eight per-system DS-3.9 audits above already independently re-verified each system's own reuse and duplication claims, including re-confirming that the two Rejected-bucket citations (Production's pipeline model, PlatformRelationshipMap/CapabilityRegistryDiagram) still exist and still match their original descriptions.";
