...
export const CERTIFICATION_JUSTIFICATION = [
  ...
  "Foundation compliance, Operational compliance, and dependency layering are all a clean 8/8 across every system — zero duplicated Foundation or Operational functionality, and zero reverse-dependency imports into the app/Platform/diagram-library tiers, in either direction. This is the tier's strongest result and the reason it clears Production Ready comfortably: 88 of the 96 non-Adoption dimension checks (11 dimensions × 8 systems) are Pass, with the remaining 5 all Partial and zero Fail.",
  "The one gap found across every system is a first-party aria-live announcement pattern. WorkflowStatus (Workflow Framework) and WorkflowStepperProgress (Workflow Stepper) now call useAnnounce() on value change, but PipelineStatus, ApprovalStatus, WorkflowSelection, and StateNode remain unannounced — 2 of 92 components covered, not the whole tier. WorkflowStepperStep's aria-current=\"step\" regression is fixed: it now computes and renders aria-current on both its button and div paths. The PipelineStage naming collision remains open. A library-wide certification requires every member system to clear the bar with zero open exceptions, not most of them.",
...
export const REMAINING_BLOCKERS: RemainingBlocker[] = [
  { toLevel: "certified", item: "Resolve or explicitly rename the WorkflowStep (type) and PipelineStage (component) naming collisions, alongside the five already-disclosed same-name cases against the plural Workflow Diagram Library and the Illustration Library." },
