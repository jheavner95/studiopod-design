...
export const PRODUCTION_CLEAN_FINDINGS: string[] = [
  ...
  "Pipeline: diagram-layer only in src/production/ (ProductionPipelineDiagram.tsx's own doc comment states \"no pipeline-specific rendering code exists anywhere in this component\") — the real reuse target is Pipeline Components' own Pipeline/PipelineStage (DS-3.5, already certified), which ProductionPipeline/ProductionStagePanel both re-export directly rather than duplicating.",
  "Queue: src/production/ has no queue-specific code beyond a single stat-tile label (\"Queue: 6\" in a health dashboard example) — the real, correct reuse target is Operational's own Queue (DS-2.5.8, already certified), which ProductionQueue re-exports directly.",
  "QA: diagram-layer only in src/production/ (QualityGateDiagram.tsx, QualitySummary.tsx — same pure-data-reducer pattern as Validation). The real reuse target is Pipeline Components' own PipelineGate composing Approval & Review's own ApprovalDecision/ApprovalStatus (DS-3.4/DS-3.5, already certified), which ProductionValidationPanel re-exports directly.",
  "Naming (found at DS-4.10 certification, not originally disclosed here): the component ProductionPipeline (src/components/platform/production/ProductionPipeline.tsx, a re-export of Pipeline Components' own Pipeline) shares its exact identifier with a pre-existing TypeScript interface, ProductionPipeline, in src/production/types/production.ts — the diagram-compilation data shape used throughout src/production/examples/ and ProductionPipelineDiagram.tsx. The two never collide at compile time (different modules, never imported together), and the type is data-only (no rendering code), but this is a real, previously-undisclosed same-name collision — the same class of finding Pipeline Components' own DS-3.5 audit already disclosed for PipelineStep vs. the Illustration Library's PipelineStep. Recorded here for parity with that precedent.",
];
