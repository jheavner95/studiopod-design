...
export const INSPECTOR_PROMOTION_CANDIDATES: InspectorPromotionCandidate[] = [
  {
    ...
    description:
      "All three wrap the motion engine's Expand primitive around an identical hand-rolled shell — a flex-col div with rounded-lg border border-border bg-surface p-4, a StatusBadge/HealthIndicator row, a muted Body description, and one or more uppercase-Caption-labeled <ul> lists of related items (Capabilities/Domains/Incoming/Outgoing for Platforms; Inputs/Outputs/Providers for Capabilities). The DS-0.2 inventory already flagged this exact recurrence (\"Inspector Panel\" status: \"Partial\") before this package existed to fix it.",
    migrationNote:
      "A real candidate for a future Foundation Adoption Pilot: the shell each one hand-rolls is exactly InspectorSection's job, and each labeled list is exactly InspectorGroup + InspectorProperty's job. Not migrated in this package, which only builds new components and does not touch src/platforms, src/capabilities, or src/workflows.",
  },
];
...
export const INSPECTOR_CLEAN_FINDINGS: string[] = [
  ...
  "The Inspector Workspace doc page (DS-1.7) itself is entirely prose/data — its own page.tsx states plainly this is documentation only, not a rendered inspector; InspectorAnatomyExplorer/VariantCard/PlatformExampleCard are all Card-based text renderers, not an inspector implementation.",
];
