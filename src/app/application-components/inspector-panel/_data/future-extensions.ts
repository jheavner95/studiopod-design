...
export const INSPECTOR_FUTURE_EXTENSIONS: InspectorFutureExtension[] = [
  ...
  { title: "AI summaries", description: "A generated natural-language summary of the selected object's state, surfaced near the Header — deferred until a real AI Operations Center package exists to own that capability." },
  { title: "Custom inspectors", description: "Letting a platform register its own InspectorSection set without touching InspectorPanel's own code — today every Inspector composition is hand-assembled per screen, which is fine at the current scale (see the Gallery below, all 8 variants hand-composed) but would need a registration API once there are many more." },
];
