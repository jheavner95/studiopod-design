...
export const BUSINESS_FEATURES_NOTE =
  "\"Business Features\" did not appear anywhere else in the codebase when this page was first written — confirmed by direct search across every prior DS-0 through DS-3 package's own data files, and this package was the first to define it. It names the tier where a real StudioPOD screen — with actual data fetching, actual business rules, actual users — would live, sitting above everything this design system itself builds. The term has since propagated correctly into every DS-4.x platform package's own promotion-candidates and implementation-guidance data (all eight consistently use it with the meaning defined here), confirmed at DS-4.10 — expected vocabulary adoption, not drift.";
