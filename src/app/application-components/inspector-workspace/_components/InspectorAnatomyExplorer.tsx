...
function RegionDetail({ region }: { region: InspectorRegion }) {
  return (
...
/** The Inspector's eight regions, stacked top to bottom exactly as the anatomy diagram shows — select one to see its full detail below. */
export function InspectorAnatomyExplorer() {
  const [selectedId, setSelectedId] = useState(INSPECTOR_REGIONS[0].id);
  const selected = INSPECTOR_REGIONS.find((region) => region.id === selectedId) ?? INSPECTOR_REGIONS[0];

  return (
    <div className="flex flex-col gap-8">
      <div className="flex flex-col gap-3 rounded-xl border border-border-subtle bg-surface/40 p-6 sm:p-10">
        {INSPECTOR_REGIONS.map((region) => (
          <RegionCard
            key={region.id}
            region={region}
