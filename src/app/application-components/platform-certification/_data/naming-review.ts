...
export const NAMING_COLLISIONS: NamingEntry[] = [
  {
    ...
    detail: "Same identifier, different module — a component (re-exporting Pipeline Components' own Pipeline) and a plain data-shape interface. No compile-time conflict (never imported together), and the type is data-only with no rendering code, but this was not previously disclosed in Production Platform's own promotion-candidates.ts. Disclosed there now, the same class of finding already disclosed in the Pipeline Components audit for PipelineStep vs. the Illustration Library's PipelineStep.",
  },
  {
  ...
  },
  {
    ...
    system: "Operations Platform, Integrations Platform",
