...
export const DS4_WORK_PACKAGES: WorkPackageSummary[] = [
  ...
  { code: "DS-4.10", title: "Platform Component Library Certification", href: "/application-components/platform-certification", oneLiner: "The DS-4 capstone — nine independent audits, five real documentation defects found and fixed in the same pass, and a Certified verdict for all eight platforms." },
];
...
export const EXECUTIVE_SUMMARY_STRENGTHS = [
  ...
  "Five real documentation defects were found across the tier during this certification — and every one was corrected in the same pass, not left as a standing gap. This is a stronger outcome than DS-3.9's own \"Production Ready, held back by tracked unresolved gaps\" result one tier down.",
...
export const EXECUTIVE_SUMMARY_WEAKNESSES = [
  "No platform implements a first-party aria-live announcement pattern — the same systemic gap DS-2.5.10 found one tier down and DS-3.9 reconfirmed one tier up, now confirmed unresolved a third tier up. Every platform discloses this honestly rather than hiding it.",
  ...
  "Zero real-screen adoption anywhere in the codebase — every one of the 96 components has only ever been exercised inside its own documentation gallery, the same structural gap DS-2.5.10 and DS-3.9 both found one and two tiers down. This tier cannot close it alone; it's the explicit subject of DS-5.",
];

export const DS4_COMPLETION_SUMMARY =
  "DS-4 set out to prove that real, usable domain-specific component libraries — the layer where business vocabulary (Order, Artwork, Content Item, Provider Connection) finally attaches to real UI — could be built entirely on top of the certified Foundation, Operational, and Workflow layers without duplicating any of them. On the evidence gathered across nine independent per-package audits, it did: eight platforms, 96 components, zero platform-boundary violations, zero reverse-dependency violations in any direction, and a shared-ownership re-export precedent mature enough that most platforms needed no new code at all. This certification found five real documentation defects along the way — three false or self-contradictory claims in three separate platform packages, one undisclosed naming collision, and a founding architecture document whose own \"nothing is built yet\" narrative had gone stale — and fixed every one in the same pass. What DS-4 could not yet prove, the same as DS-2.5 and DS-3.9 before it, is that any of this holds up under real adoption, because nothing has adopted it yet — that is DS-5's job. All eight platforms are independently Certified; the tier as a whole is Certified, held back from Enterprise Certified only by the complete absence of real Business Feature adoption — a structural fact about where DS-4 sits in the roadmap, not a defect in the library itself.";
