...
export const WORKFLOW_CLEAN_FINDINGS: string[] = [
  ...
  "Planning: no src/planning/ directory or component exists anywhere in the repo (confirmed via find -type d). \"Planning\" only appears as an architecture-diagram node label or prose reference (e.g. the DS-0.2 inventory describing itself as \"a planning inventory, not an implementation\").",
  "Intelligence: re-confirmed the DS-2.5.9 finding — no src/intelligence/ directory exists; the only real code is src/platforms/examples/intelligencePlatform.tsx, a PlatformArchitecture data object (observe/analyze/recommend/automate loop) with no steps array or stage/step shape, consumed by the platform-architecture diagram engine.",
