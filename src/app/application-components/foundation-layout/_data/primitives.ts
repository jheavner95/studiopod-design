...
export const LAYOUT_PRIMITIVES: LayoutPrimitiveDoc[] = [
  ...
  },
  {
    ...
    topics: [
      ...
      { label: "Resizable", note: "Not implemented. A drag-to-resize Panel needs pointer-tracking state this primitive intentionally doesn't own — see Future Extensions below (Resizable Panels)." },
    ...
    commonMistakes: [
      "Reimplementing Panel's border+background+header pattern by hand inside a new DS-2.x package instead of importing it.",
  ...
  },
  {
    ...
    topics: [
      ...
      { label: "Responsive", note: "layout=\"responsive\" (the default) stacks below sm: and goes side-by-side from sm: up — exactly the behavior every DS-1.x Accessibility section already hand-implemented." },
      { label: "Long values", note: "Every value cell carries min-w-0 break-words by default — the same CSS Grid blowout fix first diagnosed in DS-0.3, now built into the primitive instead of re-applied by hand each time." },
    ...
    usage: "<DescriptionList items={[{ label: \"...\", value: \"...\" }]} /> — replaces the bordered dl/dt/dd block hand-rolled identically across every DS-1.x Accessibility section.",
    ...
    commonMistakes: [
      "Hand-rolling the dl/dt/dd + border-b + first:pt-0 block again in a new DS-2.x page instead of importing DescriptionList — the exact duplication this primitive exists to end.",
