...
export const DEPENDENCY_FUTURE_EXTENSIONS: DependencyFutureExtension[] = [
  ...
  { title: "Impact simulation", description: "Previewing which nodes would become Blocked or Critical if a given node were removed, before actually removing it — deferred pending real usage, the same stance Pipeline Components' own future-extensions already took on simulation mode." },
  ...
  { title: "Dependency replay", description: "Re-running how a graph's own connectivity changed over time, step by step, rather than the static current-state view this package renders today — the same replay concept State Machine's own future-extensions already deferred for its own history." },
  { title: "AI relationship analysis", description: "Flagging an unusual or risky dependency pattern (e.g. a newly-introduced cycle) rather than a human reading DependencyLegend/DependencySummary themselves — deferred pending real usage, the same stance every prior Workflow Component Library package's own future-extensions already took on AI assistance." },
];
