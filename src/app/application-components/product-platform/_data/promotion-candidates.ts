...
export const PRODUCT_CLEAN_FINDINGS: string[] = [
  ...
  "Library: fully covered by the existing generic Asset Browser. The one \"Product Library\" hit (a gallery demo card in Asset Browser's own gallery) is built entirely on the generic, type-parameterized AssetBrowser<T> component with zero product awareness — the same generic surface this package's own ProductLibrary re-exports directly. Asset Browser's own migration-notes audit already confirmed no hand-rolled product-library browsing code exists anywhere in the repo.",
];
