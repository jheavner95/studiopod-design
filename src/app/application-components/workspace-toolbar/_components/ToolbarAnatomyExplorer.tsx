"use client";

import { useState } from "react";
import Link from "next/link";
import { ArrowUpRight } from "lucide-react";
import { cn } from "@/lib/utils";
import { Card, Badge, Body, Caption, Heading } from "@/components/ui";
import { Activate } from "@/motion";
import { TOOLBAR_REGIONS, type ToolbarRegion } from "../_data/regions";

interface RegionCardProps {
  region: ToolbarRegion;
  selected: boolean;
  onSelect: () => void;
}

function RegionCard({ region, selected, onSelect }: RegionCardProps) {
  return (
    <button type="button" onClick={onSelect} aria-pressed={selected} className="focus-ring block w-full rounded-lg text-left">
      <Activate state={selected ? "active" : "inactive"} className="rounded-lg">
        <Card
          interactive
          padding="sm"
          className={cn("flex h-full min-h-24 flex-col gap-1.5", selected && "border-accent-500/60 bg-accent-soft/30")}
        >
          <span className="text-body-sm font-medium text-ink-primary">{region.name}</span>
          <Body size="sm" muted className="line-clamp-2">
            {region.purpose}
          </Body>
        </Card>
      </Activate>
    </button>
  );
}

function RegionDetail({ region }: { region: ToolbarRegion }) {
  return (
    <Card padding="lg" className="flex flex-col gap-6">
      <div className="flex flex-col gap-2">
        <Heading level={3}>{region.name}</Heading>
        <Body muted>{region.purpose}</Body>
      </div>

      <div className="flex flex-col gap-2">
        <Caption className="text-ink-tertiary">Examples</Caption>
        <div className="flex flex-wrap gap-2">
          {region.examples.map((example) => (
            <Badge key={example} tone="accent" size="sm">
              {example}
            </Badge>
          ))}
        </div>
      </div>

      <div className="flex flex-col gap-4 border-t border-border-subtle pt-4">
        {region.guidance.map((note) => (
          <div key={note.label} className="flex flex-col gap-1.5">
            <Caption className="text-ink-tertiary">{note.label}</Caption>
            <Body size="sm" muted>
              {note.text}
            </Body>
          </div>
        ))}
      </div>

      <div className="flex flex-col gap-2 border-t border-border-subtle pt-4">
        <Caption className="text-ink-tertiary">Reuse notes</Caption>
        <Body size="sm" muted>
          {region.reuseNotes}
        </Body>
        {region.reuseLinks.length > 0 ? (
          <div className="flex flex-wrap gap-4 pt-1">
            {region.reuseLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className="focus-ring flex items-center gap-1 rounded-md text-caption font-medium text-accent-400 transition-colors duration-[var(--duration-fast)] ease-[var(--ease-standard)] hover:text-accent-300"
              >
                {link.label}
                <ArrowUpRight className="size-3.5" aria-hidden />
              </Link>
            ))}
          </div>
        ) : null}
      </div>
    </Card>
  );
}

/** The toolbar's eight regions, exploded into one responsive row — select one to see its full detail below. */
export function ToolbarAnatomyExplorer() {
  const [selectedId, setSelectedId] = useState(TOOLBAR_REGIONS[0].id);
  const selected = TOOLBAR_REGIONS.find((region) => region.id === selectedId) ?? TOOLBAR_REGIONS[0];

  return (
    <div className="flex flex-col gap-8">
      <div className="rounded-xl border border-border-subtle bg-surface/40 p-6 sm:p-10">
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4 lg:grid-cols-8">
          {TOOLBAR_REGIONS.map((region) => (
            <RegionCard
              key={region.id}
              region={region}
              selected={region.id === selectedId}
              onSelect={() => setSelectedId(region.id)}
            />
          ))}
        </div>
      </div>

      <RegionDetail region={selected} />
    </div>
  );
}
