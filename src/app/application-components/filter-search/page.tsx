...
function CrossLinks() {
  const links = [
    ...
    { label: "Asset Browser", href: "/application-components/asset-browser" },
  ];
  return (
  ...
  return (
    <PageShell background={<SystemGrid />}>
      <SectionShell spacing="xl">
        <PageIntro
          eyebrow="package · application components · filter & search"
          title="Filter & search system"
          description="The canonical StudioPOD Filter & Search System — the standard discovery experience used throughout every StudioPOD library, queue, and operational workspace. Composes Foundation Forms/Metadata/Navigation/Overlay/Feedback end to end, and Operational Data Grid and Asset Browser directly, rather than duplicating any of them. Built in DS-2.5.5; not yet adopted by any real screen."
        >
          <div className="flex flex-wrap items-center gap-4 pt-2">
            <Badge tone="warning" size="sm" className="w-fit">
              Production Ready — DS-2.5.5
            </Badge>
          </div>
          <div className="pt-2">
            <CrossLinks />
      ...
      <SectionShell spacing="lg" divider>
        <div className="flex flex-col gap-10">
          <SectionHeader
            eyebrow={<Eyebrow tone="accent">Filter & search anatomy</Eyebrow>}
        ...
        <div className="flex flex-col gap-10">
          ...
          {SEARCH_PROMOTION_CANDIDATES.length === 0 ? (
            <Card className="flex flex-col gap-2 border-success/30 bg-success-soft">
              ...
              <Body size="sm" muted>
                No existing hand-rolled search/filter/sort UI was found anywhere in the codebase across all eight named domains, including a fresh re-check of this repo&rsquo;s own four MS-2.x Library Playground pages. See the clean findings below for what was actually checked — and one real, pre-existing code duplication this package itself resolved (Data Grid&rsquo;s and Asset Browser&rsquo;s pagination components each inlined an identical &ldquo;X–Y of Z&rdquo; calculation; both now delegate to this system&rsquo;s own ResultSummary).
              </Body>
            </Card>
          ) : null}
