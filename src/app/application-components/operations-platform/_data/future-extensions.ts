...
export const OPERATIONS_FUTURE_EXTENSIONS: OperationsFutureExtension[] = [
  ...
  { title: "Automation orchestration", description: "Real scheduled-trigger and workflow-execution logic behind OperationsAutomation's own stage display — this audit confirmed no real automation-execution engine exists anywhere in the repo (see Promotion Candidates), so this is genuinely greenfield, deferred pending real usage." },
  ...
  { title: "Capacity planning", description: "Forecasting resource needs from OperationsMonitoring's own historical data — belongs to the Intelligence platform template (DS-4.1's own architecture), not this one; deferred here rather than duplicated." },
