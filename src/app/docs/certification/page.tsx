...
const primaryEntryPoints = capstones.filter(
  (e) => e.id === "workspace-certification" || e.id === "foundation-audit" || e.id === "application-composition-certification",
);
...
export default function DocsCertificationPage() {
  return (
    <DocsShell entry={entry} toc={<DocsTableOfContents />}>
      ...
      <DocsSectionLanding
        purpose="Certification is the cross-cutting rollup that closes out each tier of the design system — one capstone review per tier, from the Workspace shell up through Application Composition. Each capstone independently re-audits its tier's own real source rather than trusting that tier's own docs pages, and publishes a verdict, a scorecard, and any real defects it found along the way."
        whatYoullLearn={[
          "The six-capstone structure, one per tier: Workspace, Foundation, Operational, Workflow, Platform, and Application Composition.",
          ...
          "Where each capstone found and fixed real defects during its own audit pass — for example, five documentation defects fixed during Platform Certification, and two real defects resolved during Application Composition Certification.",
        ]}
        stats={STATS}
