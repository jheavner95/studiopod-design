...
export const REVIEWED_PACKAGES: ReviewedPackage[] = [
  {
    ...
    summary: "The canonical composition chain (Foundation → Operational → Workflow → Platform → Business Features → Application), the Business Feature trait/anatomy/template model, composition rules, and a grep-grounded audit of ten candidate features.",
  },
  {
    code: "DS-5.2",
    title: "Business Feature Composition Framework",
    href: "/docs/business-features",
    summary: "The ten-part Feature Architecture ownership tree, a generic thirteen-part Feature Template, nine reusable Feature Categories, composition rules, a seven-stage lifecycle, and a category mapping for all ten candidates.",
  },
  {
    code: "DS-5.3",
...
export const VERIFY_DIMENSIONS: VerifyDimension[] = [
  { dimension: "Architecture consistency", verdict: "Pass", finding: "DS-5.2, DS-5.3, and DS-5.4 all reuse DS-5.1's own vocabulary verbatim (Feature Architecture, Composition Stack, Business Feature Model) rather than each coining a competing term for the same concept." },
  { dimension: "Business Feature framework compliance", verdict: "Pass", finding: "The pilot's own nine components plus hook plus mock-data file map one-to-one onto DS-5.2's own ten-part Feature Architecture — confirmed field-by-field in DS-5.4's own Feature Anatomy section, re-verified here against the actual file tree." },
  { dimension: "Template compliance", verdict: "Partial", finding: "The pilot's Workspace shell, Navigation, Primary content, Inspector, Commands, Metrics, and Actions all match DS-5.3's own Workspace Feature Template exactly. One gap: DS-5.3's own seven-part template list has no separately named \"Dialogs\" entry, though the pilot correctly built five dialogs per DS-5.2's own thirteen-part generic template. See Promotion Review." },
  ...
  { dimension: "Workflow usage", verdict: "Pass", finding: "WorkflowStep and WorkflowFooter are composed directly — correct, since neither has a Platform-tier re-export, matching DS-5.2's own Composition Rules allowance for composing Workflow directly when no Platform-tier equivalent exists." },
  ...
  { dimension: "Naming", verdict: "Pass", finding: "Zero collisions found — a direct grep for every ProductionFeature* component name against the rest of src/components/ returns no matches, unlike DS-4.10's own disclosed ProductionPipeline collision one tier down." },
  ...
  { dimension: "Validation ownership", verdict: "Pass", finding: "The Draft → Ready → Validating → Validated → Complete flow and its mapping into Workflow's own ApprovalStateValue vocabulary are both feature-owned — Workflow has no such flow built in, confirmed by direct source read during DS-5.4's own research phase." },
  ...
  { dimension: "Adoption readiness", verdict: "Partial", finding: "Exactly one Business Feature exists (this pilot), and it is explicitly non-production by its own scope (mock data, local state, no real repositories or APIs). Real, if not yet production, adoption evidence now exists — an improvement over DS-4.10's own Fail verdict on this same dimension one tier down, but not yet a production feature." },
];
