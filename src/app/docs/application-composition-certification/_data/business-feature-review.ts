...
export const BUSINESS_FEATURE_CHECKS: BusinessFeatureCheck[] = [
  ...
  { part: "Dialogs", verdict: "Pass", evidence: "ProductionFeatureDialogs.tsx implements all five named dialogs (Validation, Delete, Publish, Export, Confirmation), switching on one piece of hook-owned state — the Advance Stage confirmation flow was manually exercised end to end in DS-5.4's own browser QA." },
  ...
  { part: "Actions", verdict: "Pass", evidence: "Distinct from Commands only in naming — ProductionFeatureActions.tsx is the single component serving both; DS-5.2's own Feature Architecture treats Commands and Actions as the same concern for a feature this size, and the pilot follows that." },
