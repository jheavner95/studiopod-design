...
export const ADOPTION_TARGETS: FeatureAdoptionEntry[] = [
  ...
  },
  {
    ...
    finding: "No dedicated component exists anywhere. A repo-wide grep found only incidental hits: the earlier, pre-Platform-tier illustration-canvas Platform Architecture Library's own example data (src/platforms/examples/), unrelated \"planning\" prose in Workflow gallery copy, and the historical discrepancy Platform Architecture's own Adoption Targets already flagged — workflow-certification's roadmap.ts named \"Planning\" as a platform with nothing built anywhere. That gap is still open. A Planning Business Feature has no Platform-tier shell to compose yet; it would need a new platform-tier package first, or compose Workflow's own Pipeline Components / State Machine directly with no Platform-tier layer in between.",
  },
  {
    candidate: "Creative",
    verdict: "no-platform-coverage",
    finding: "No dedicated component exists anywhere. A repo-wide grep found only incidental hits, all inside the earlier, pre-Platform-tier illustration-canvas Platform Architecture Library (src/platforms/types/platform.ts, src/platforms/components/PlatformFlowDiagram.tsx) and Workflow Visualization's own gallery copy — none of it real creative-tooling logic. Distinct from Intelligence (which does have a Certified Platform), Creative is a genuinely open gap with no Platform-tier coverage at all.",
  },
];

export const ADOPTION_SUMMARY =
  "Six of the ten prospective features (Production, Products, Publishing, Commerce, Intelligence, Administration) already have a Certified Platform-tier component library to compose — the Business Feature work for each is architecture-ready today. Two (Settings, Automation) have real but partial Platform coverage through a sibling platform's own component (Admin's AdminConfiguration, Operations' OperationsAutomation) rather than a dedicated platform of their own. Two (Planning, Creative) have no Platform-tier coverage at all — confirmed by direct grep, not assumed from the name — and would need new Platform-tier work, most likely a future platform-tier package, before a Business Feature could compose them the same way the other eight do.";
