...
export const FEATURE_STRUCTURE: StructurePart[] = [
  {
    ...
    kind: "root",
    owner: "—",
    description: "The whole — one user-facing, domain-specific capability (DS-5.1's own Business Feature Model). Every part below is internal to it.",
  },
  {
    name: "Workspace",
    kind: "ui",
    owner: "Business Feature (composition)",
    description: "The feature's own outer shell — composes DS-1's own six-region Workspace Architecture blueprint scoped to this one feature, the same way a Platform's own Platform Shell does one layer down.",
  },
  {
    name: "Views",
    kind: "ui",
  ...
  },
  {
    ...
    kind: "data",
    owner: "Business Feature",
    description: "Data-access abstractions, however a feature chooses to talk to its own backend — see DS-5.1's own Application Boundaries. No certified tier below Business Feature has ever contained a repository or a fetch call.",
  },
  {
    name: "State",
    kind: "data",
    owner: "Business Feature (local), Application (shared)",
    description: "Local state (selection, form drafts, view mode) is owned by the feature. State shared across more than one feature — current user, feature flags — is owned by the Application shell instead, per DS-5.1's own Boundaries answer.",
  },
  {
    name: "Validation",
    kind: "data",
    owner: "Foundation (field-level), Business Feature (business-rule-level)",
    description: "Generic field validation is owned by Foundation's own Form System. Whether a value is valid for this domain's own business rules is owned by the Business Feature, per DS-5.1's own Boundaries answer.",
  },
  {
    name: "API",
    kind: "data",
    owner: "Business Feature",
    description: "Which endpoints a feature calls and how it shapes requests/responses, per DS-5.1's own Boundaries answer — the same feature-owned boundary as Repositories.",
  },
];
