...
export function DocsPageNavigation({ entry, className }: DocsPageNavigationProps) {
  ...
  return (
    <div className={cn("grid grid-cols-1 gap-4 sm:grid-cols-2", className)}>
      {previous ? (
        <Link href={previous.href} className="focus-ring block rounded-lg">
      ...
      {next ? (
        <Link href={next.href} className="focus-ring block rounded-lg">
          <Card interactive className="flex h-full flex-col items-end gap-2 text-right">
            <Inline gap="xs" align="center" className="text-ink-tertiary">
