 ...
 * declared in apps/docs/src/lib/certification.ts's CERTIFICATION_CHECKLIST. These are
 * plain functions over the filesystem, exercised by
 * apps/docs/src/lib/certification.test.ts via Vitest (which already runs in Node, not
 ...
 * (packages/design/scripts/check-api.mjs), not hand-maintained —
...
export function checkExportStatus(componentName: string): ExportStatusResult {
  const baselineDir = join(REPO_ROOT, "packages/design/api-baseline");
...
export function checkVisualBaseline(componentName: string): VisualBaselineResult {
  const visualDir = join(REPO_ROOT, "apps/docs/e2e/visual");
