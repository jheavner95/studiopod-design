...
export const IMPLEMENTATION_GUIDANCE: PublishingImplementationGuidance[] = [
  { label: "Platform composition", text: "Every one of these 12 components is a pure re-export over an already-certified Workflow or Operational component — checked directly against Platform Architecture's own composition rules before writing each file, and in every case an existing component's prop surface already covered the need with no Publishing-specific field required. The same zero-new-wrapper-code precedent Product Platform Components already established." },
