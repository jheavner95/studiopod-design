...
export const PLATFORM_TEMPLATES: PlatformTemplate[] = [
  {
    ...
    workflowUsage: "Pipeline Components for the production run itself (stage/gate progression); State Machine for a single artifact's own lifecycle; Dependency & Relationship Views for artwork/font dependency checks — the same domain the Dependency & Relationship Views gallery already demos (\"Artwork Dependencies\").",
  ...
  },
  {
    ...
    workflowUsage: "State Machine for order lifecycle — the same domain the State Machine gallery already demos (\"Commerce Order Flow\"); Approval & Review for refund or high-value-order approval; Pipeline Components for the fulfillment pipeline.",
  ...
  },
  {
    ...
    workspaceModel: "Status Workspace (Workspace Architecture's own operational-status tier) as the primary region, composing a dashboard of Platform-level metrics.",
    workflowUsage: "Dependency & Relationship Views for cross-platform impact analysis — the same domain the Dependency & Relationship Views gallery already demos (\"Cross-platform Impact\").",
