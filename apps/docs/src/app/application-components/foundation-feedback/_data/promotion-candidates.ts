...
export const FEEDBACK_PROMOTION_CANDIDATES: FeedbackPromotionCandidate[] = [
  {
    ...
    description:
      "A real, already-shipped instance of exactly the Banner shape (role=\"status\", message + primary/discard actions) — built directly on Surface + Inline rather than this system's new Banner primitive. Not duplicated anywhere else: grep -rln \"UnsavedChangesBanner\" src/app returns zero matches, so it's a single bespoke instance, not a pattern repeated across pages.",
    migrationNote:
      "A one-file option for a future adoption pilot to rebuild on Banner, not a multi-file duplication problem. Left untouched here, since this pass only builds new components and does not migrate existing ones.",
  },
];
...
export const FEEDBACK_CLEAN_FINDINGS: string[] = [
  ...
  "Validation panels: foundation-forms/_components/ValidationDemo.tsx correctly imports the real ValidationSummary from @/components/form; every other hit uses the unrelated Production system's ValidationDiagram/ValidationTimeline/ValidationLegend (pipeline-stage visualization, not a validation-list UI).",
  ...
  "Notifications: no notification-list/toast UI exists anywhere in src/app to duplicate — the Operational Status Notifications region and the Foundation Component Catalog's toast/alert entries were, until this system shipped, purely planned (status: \"Needed\"), not built and copy-pasted.",
];
