...
export const WORKFLOW_ACCESSIBILITY_TOPICS: WorkflowAccessibilityTopic[] = [
  ...
  {
    label: "ARIA",
    text: "WorkflowStatus inherits StatusIndicator's own dot+label semantics unchanged; WorkflowProgress inherits ProgressBar's real role=\"progressbar\"/aria-valuenow unchanged — this family adds no new ARIA roles of its own, only compositions of existing ones.",
  },
  {
    label: "Status announcements",
    text: "WorkflowStatus announces its own transition (e.g. \"Workflow status: Blocked.\") through the shared LiveRegionProvider mounted at the app root whenever its value prop changes — blocked and failed announce assertively, every other state announces politely.",
  },
  {
